...
double LatticeBoltzman::C(int ix,int iy){
    double n_refrac=1.0; double n_lente=1.33; double n_Aux;
    int W, ix_left, ix_right; W=32; ix_left=64; ix_right=ix_left+W;
    double p=(n_lente-1)/2.0;
    double p2=n_lente+1;
    double ixc=5, iyc=69, R=91, R2=R*R, iy2=iyc*iyc;

    
    
    if (ix_left < ix && ix <= ix_left+W/2 && (ix-ixc)*(ix-ixc)+(iy-iyc)*(iy-iyc)<=R2){
        n_Aux=1+p*(1+tanh(ix-64));
    }
    else if (ix_left+W/2 < ix && (ix-ixc)*(ix-ixc)+(iy-iyc)*(iy-iyc)<=R2){
        n_Aux=p2-(1+p*(1+tanh(ix-96)));
    }  
    else {
        n_Aux=n_refrac;
    }
...
double LatticeBoltzman::feq(double rho0, double Jx0, double Jy0, int ix, int iy, int i){
...
void LatticeBoltzman::ImponerCampos(int t){
    int i, ix, iy, n0;
    double lambda, A, omega, rho0, Jx0, Jy0; A=10; lambda=10; omega=2*M_PI/lambda*C(ix,iy);
...
void LatticeBoltzman::Print(const char * NameFile){
    ofstream MyFile(NameFile); double rho0, C0; int ix, iy;
    for(ix=0;ix<Lx;ix++){
        for(iy=0;iy<Ly;iy++){
            C0=C(ix,iy);                                
            rho0=rho(ix,iy,true);
