...
int main() {
	...
	double v2x = -v1 * sin(60 * M_PI / 180);
	double v2y = v1 * cos(60 * M_PI / 180);
	...
	for (double t = 0; t < Ttotal; t += dt) {
		...
		rottroj[0] = bodies[2].Getx() * std::cos(-angle) - bodies[2].Gety() * std::sin(-angle);
		rottroj[1] = bodies[2].Getx() * std::sin(-angle) + bodies[2].Gety() * std::cos(-angle);
